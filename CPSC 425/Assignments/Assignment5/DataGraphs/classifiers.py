...
def nearest_neighbor_classify(train_image_feats, train_labels, test_image_feats):
    ...
    '''
     # perform kmn as according to scikit example
    neighbor = KNeighborsClassifier(n_neighbors=20)
    neighbor.fit(train_image_feats, train_labels)
    predicted_labels = neighbor.predict(test_image_feats[0])
...
def svm_classify(train_image_feats, train_labels, test_image_feats):
    ...
    '''
    # perform svm as according to scikit example
    clf = svm.SVC(kernel = 'linear', C = 158 ) # linear SVC
    clf.fit(train_image_feats, train_labels) 
    predicted_labels = clf.predict(test_image_feats[0]) 
    return predicted_labels
